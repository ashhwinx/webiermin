...
const Navbar = () => {
  ...
  useEffect(() => {
    const sections = ['hero', ...navLinks.map((link) => link.toLowerCase()), 'contact'];
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    gsap.to(spotlightRef.current, { x: x, opacity: 1, scale: 1, duration: 0.4 });
  };
            ...
            >
              <span
                className={`text-sm font-bold uppercase tracking-wider ${
                  activeSection === link.toLowerCase() ? 'text-[#3533cd]' : 'text-gray-800'
                }`}
              >
                {link}
              </span>
...
// Fix 3: Interface for Props
interface MagneticItemProps {
  children: React.ReactElement;
}

const MagneticItem = ({ children }: MagneticItemProps) => {
  // Fix 4: Type the ref as HTMLElement so it can accept divs/anchors
  ...
  useEffect(() => {
    ...
    // Fix 5: Use native MouseEvent for addEventListener
  ...
  return React.cloneElement(children, { 
    // @ts-ignore - ref injection in cloneElement is tricky in TS, ignoring is safe here
    ref: magnetic 
  });
};
